"""One-shot validation of candidate Food & Beverage default-source RSS URLs.

Mirrors the backend's `_build_custom_source_from_url` logic (requests.get with a
20s timeout + a browser UA, then feedparser) so the result predicts what the
production validate endpoint would do for each URL. The Food & Beverage topic is
deliberately food/cooking/drinks-focused — distinct from the existing
"Food & Travel" topic, which leans on destinations and dining-out.

Run:
    python scripts/validate_candidate_sources_food_beverage.py
"""
...
CANDIDATES: dict[str, list[tuple[str, str]]] = {
    "Food & Beverage": [
        ...
        ("The Splendid Table", "https://www.splendidtable.org/feed"),
        ("Vittles", "https://www.vittlesmagazine.com/feed"),
        # --- Beverage: coffee, wine, beer, spirits, cocktails ---
        ...
        ("Sprudge (Coffee)", "https://sprudge.com/feed"),
        ("Good Beer Hunting", "https://www.goodbeerhunting.com/blog?format=rss"),
    ],
}
...
def validate(grouping: str, name: str, url: str) -> Result:
    ...
    except Exception as e:  # noqa: BLE001 - validator should never crash on one feed
        return Result(grouping, name, url, False, 0, "", f"{type(e).__name__}: {e}"[:120], int((time.time() - t0) * 1000))
